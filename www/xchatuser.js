...
async function loadTurnConfig() {
  try {
    ...
    if (response.ok) {
      ...
      window.fgdx_configuration.iceServers = turnConfig.iceServers
      console.log("TURN configuration loaded:", turnConfig.iceServers)
    } else {
      console.warn("Failed to load TURN configuration, using default STUN only")
    }
  } catch (error) {
    console.warn("Error loading TURN configuration:", error.message)
  }
}
...
class XChatUser {
  ...
  getConnectionState() {
    ...
    if (this.rtcConn.connectionstate) {
      ...
      if (
        this.rtcConn.iceConnectionState === "connected" ||
        this.rtcConn.iceConnectionState === "completed"
      ) {
  ...
  async setRemoteSdp(target) {
    if (
      this.rtcConn.signalingState === "have-local-offer" &&
      !this.rtcConn.remoteDescription
    ) {
      // console.log('setRemoteDescription', target);
      try {
        this.rtcConn
          .setRemoteDescription({ type: "answer", sdp: target })
        ...
        if (typeof message === "string") {
          if (message.startsWith("##FILE_S##")) {
            ...
            this.#totalChunks = Math.ceil(
              this.#expectedFileSize / this.#chunkSize
            )
            ...
            if (missingChunks.length > 0) {
              console.log(
                `Missing chunks: ${missingChunks.length}, requesting retry...`
              )
              // 请求重传缺失的块
            ...
            if (this.receivedSize === this.#expectedFileSize) {
              try {
                const validChunks = this.receivedChunks.filter(
                  (chunk) => chunk !== null
                )
              ...
              console.error(
                `File size mismatch: expected ${this.#expectedFileSize}, got ${
                  this.receivedSize
                }`
              )
              // 重新请求所有缺失的块
            ...
            try {
              ...
              if (parsed.type === "##CHUNK_INFO##") {
                ...
                // 处理重传请求
                console.log(
                  `Received retry request for ${parsed.chunks.length} chunks`
                )
    ...
    return new Promise((resolve) => {
      if (this.chatChannel.bufferedAmount > maxBufferedAmount) {
        ...
        const handleBufferedAmountLow = () => {
          this.chatChannel.removeEventListener(
            "bufferedamountlow",
            handleBufferedAmountLow
          )
          resolve()
        }
        this.chatChannel.addEventListener(
          "bufferedamountlow",
          handleBufferedAmountLow
        )
      } else {
